...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('cars.show', $car) }}">{{ $car->full_description }}</a></li>
    <li class="breadcrumb-item active" aria-current="page">Edit</li>
@endsection

@section('page-header')
    <h1 class="page-title"><i class="fas fa-car me-2"></i>Edit Vehicle</h1>
...
<div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="vehicle_type" 
                                ...
                                <option value="wagon" {{ old('vehicle_type', $car->vehicle_type) == 'wagon' ? 'selected' : '' }}>Wagon</option>
                                <option value="truck" {{ old('vehicle_type', $car->vehicle_type) == 'truck' ? 'selected' : '' }}>Truck</option>
                                <option value="van" {{ old('vehicle_type', $car->vehicle_type) == 'van' ? 'selected' : '' }}>Van</option>
                                <option value="motorcycle" {{ old('vehicle_type', $car->vehicle_type) == 'motorcycle' ? 'selected' : '' }}>Motorcycle</option>
                                <option value="other" {{ old('vehicle_type', $car->vehicle_type) == 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                            @error('vehicle_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
    ...
    <div class="col-lg-4">
        <div class="card">
            ...
            <div class="card-body">
                <p class="mb-2"><strong>Created:</strong> {{ $car->created_at->format('M d, Y') }}</p>
                <p class="mb-2"><strong>Last Updated:</strong> {{ $car->updated_at->format('M d, Y') }}</p>
                <p class="mb-0"><strong>Database ID:</strong> #{{ $car->id }}</p>
            </div>
        </div>
        ...
        <div class="card mt-3">
            <div class="card-header">
                <i class="fas fa-file-contract text-success"></i> Active Policies
            </div>
            <div class="card-body">
                <p class="small text-muted mb-2">This vehicle has {{ $car->policies->count() }} insurance policy(ies).</p>
                <a href="{{ route('policies.index', ['car_id' => $car->id]) }}" class="btn btn-sm btn-outline-primary">
