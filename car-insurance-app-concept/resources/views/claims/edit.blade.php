...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('claims.show', $claim) }}">{{ $claim->claim_number }}</a></li>
    <li class="breadcrumb-item active" aria-current="page">Edit</li>
@endsection

@section('page-header')
    <h1 class="page-title"><i class="fas fa-clipboard-list me-2"></i>Edit Claim</h1>
    <p class="page-subtitle">Update claim information</p>
@endsection
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('claims.update', $claim) }}" method="POST">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="policy_id" 
                                ...
                                @foreach($policies as $policy)
                                    <option value="{{ $policy->id }}" {{ old('policy_id', $claim->policy_id) == $policy->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="driver_id" 
                                ...
                                @foreach($drivers as $driver)
                                    <option value="{{ $driver->id }}" {{ old('driver_id', $claim->driver_id) == $driver->id ? 'selected' : '' }}>
                                        {{ $driver->full_name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="incident_date" class="form-label">
                                <i class="fas fa-calendar-alt me-1"></i> Incident Date <span class="text-danger">*</span>
                            </label>
                            <input type="datetime-local" 
                                   name="incident_date" 
                                   id="incident_date"
                                   class="form-control @error('incident_date') is-invalid @enderror" 
                                   value="{{ old('incident_date', $claim->incident_date->format('Y-m-d\TH:i')) }}"
                                   required>
                            @error('incident_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('location', $claim->location) }}">
                            @error('location')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('police_report_number', $claim->police_report_number) }}">
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="damage_amount" class="form-label">
                                <i class="fas fa-dollar-sign me-1"></i> Damage Amount
                            </label>
                            <input type="number" 
                                   name="damage_amount" 
                                   id="damage_amount"
                                   class="form-control @error('damage_amount') is-invalid @enderror" 
                                   value="{{ old('damage_amount', $claim->damage_amount) }}"
                                   step="0.01"
                                   min="0">
                            @error('damage_amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        <label for="adjuster_notes" class="form-label">
                            <i class="fas fa-sticky-note me-1"></i> Adjuster Notes
                        </label>
                        <textarea name="adjuster_notes" 
                                  id="adjuster_notes"
                                  class="form-control @error('adjuster_notes') is-invalid @enderror" 
                                  rows="3">{{ old('adjuster_notes', $claim->adjuster_notes) }}</textarea>
                        @error('adjuster_notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
